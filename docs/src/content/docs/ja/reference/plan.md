...
1. **`-cover` を使わないテストごとのカバレッジ。** 当初の計画は `-test.coverprofile` による
   ...
   だけである。ブロックカバレッジは後で別の `Criterion` として追加できる。
...
2. mutant サイトのないコード向けのブロックカバレッジ `Criterion`。テストごとのカバープロファイルを
   ビルド非依存に得る手段 (`bazel coverage` 外での rules_go の計装ビルド) ができてから行う。
3. ユーザーにシャードのレポートをまとめて渡してもらう代わりに、Bazel のテストツリー内でそれらを
