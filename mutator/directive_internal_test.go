...
`
	...
	for _, tc := range []struct {
		...
		{line: 6, op: "logical", rule: "disable", reason: "tab separated"},    // the block starts on its own line
		{line: 8, op: "logical", rule: "disable", reason: "tab separated"},    // "all" covers every operator
		{line: 8, op: "relational", rule: "disable", reason: "tab separated"}, // the first match wins
		{line: 10, op: "relational"},                                          // one enable closes both blocks
		{line: 11, op: "constant", rule: "disable-next-line"},                 // disable-next-line, no reason
		{line: 11, op: "logical"},                                             // and only for the named operator
	} {
		rule, reason := dis.find(tc.line, tc.op)
		if rule != tc.rule || reason != tc.reason {
			t.Errorf("find(%d, %q) = %q, %q; want %q, %q", tc.line, tc.op, rule, reason, tc.rule, tc.reason)
		}
	}
}
