...
2017: operator selection and stratified sampling are at best marginally better than
uniform random sampling, and a ~10% sample loses under a point of score).
`totals.sampled` records the fraction actually kept.

IDs are content hashes, and the hash covers the seed, so the selection needs no
