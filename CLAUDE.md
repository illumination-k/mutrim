...
| `ingest`   | Other languages' tool output into observations (rows of `sites` / `blocks` / `kills` by test name) for `minimize`: Stryker JSON, cargo-mutants `mutants.out` (libtest / nextest logs), Istanbul `coverage-final.json`, llvm-cov JSON export (lines, Rust demangling for `-exclude-fn`), JUnit durations; cargo-mutants places mutants for weak spots                | stdlib only                         |
...
- `run -sample <fraction> [-seed N]` (the `sample` / `seed` attributes of `mutation_test`)
  keeps a fraction of the mutants, selected by `hash(id, seed)` below it: the same mutants in
  every shard and every incremental run with the same seed (issue #24; Gopinath et al., ICSE
  2015/2016, TSE 2017: uniform random sampling, ~10% sample under a point of score). The
  unselected are `SKIPPED`, the score is computed over the sample, and `totals.sampled`
  records the fraction kept.
...
- A test failing without a mutant stops the run unless `run -skip-failing` (the
  `skip_failing` attribute of `mutation_test`): then it is `status: "failing"` in `tests[]`,
  reaches nothing and runs against no mutant, the baseline is rerun with its failing
  top-level tests `-test.skip`ped until it passes (a panic hides the tests after it), and
  `minimize` drops it from the matrix and lists it in `excluded`. No passing test is an error.
...
exported JSON matrix (`-matrix`) + an external MIP solver. `run -sample` leaves the
minimizer's inputs alone (a kept mutant's kills are still kills), but the requirements
it left out may have changed the verdicts, so a redundant test is weaker under it.
